<template>
    <section>

      <h1 class="title">Finalizar pagamento</h1>
      <hr style="width:70px; background-color:#ff0000">
      
      <p class= "is-size-6">Nosso modelo de pagamento por enquanto é o pix, estamos em fase de desenvolvimento e logo teremos outras formas para melhor atende-los. <br></p>
      <br>
      <br>
    
      <h1 class="title has-text-centered"> PIX: 106.576.769-26 </h1>

      <br>
      <a>
        <button class="button is-danger">Continuar</button>
      </a>

    </section>
</template>
